...
面向中望 CAD 和 AutoCAD 的批量打印插件。本仓库维护基于 [Edwardhehe/batchPrintZWCAD](https://github.com/Edwardhehe/batchPrintZWCAD) v1.15.7.5 的扩展版本，保留上游历史、作者信息和 MIT 许可。当前扩展版本：**2026-09-22 修订5**（程序集基础版本仍为 1.15.7.5）。

## 修订5修复

修复无有效块定义的参照导致图名/图号读取整体失败、编辑图框报 eNullObjectId 的问题。现在跳过该参照，继续读取正常图框。AutoCAD 2023 图纸副本回归：修复前3项失败，修复后文字缓存、31个图框字段识别及编辑定位3项通过；不修改原DWG。本次未重复完整打印及中望宿主回归。

## 修订4新增

- **图片签章**：仅用于“批打到文件（图框型）”。新增或编辑图框时框选签章区域，后续同类图框在该位置加章；支持缩放、旋转图框。签章叠加到输出文件，不向原 DWG 写入印章。
- **印章库**：录入 PNG/JPG/BMP、预览、改名、启停、移出和导入导出。便携 `.lastamps.json` 内嵌图片，不依赖原图片路径；不兼容 MSteel 专有印章库。
- **输出**：带章单页 PDF、合并多页 PDF、PNG/JPG。通用矩形批打、单张打印、DWF 和 DWG 拆图没有签章入口。
- **通用型清单**：命名前后缀、起始编号、补零位数、名称预览、空间排序、小图框面积过滤，以及图面尺寸、比例和备注显示。过滤按每个文件/空间的最大图框面积计算。
- **相邻纸张切换**：支持 A4→A3、A3→A2 等候选，并计算相应比例，不再只提供跨两级纸张。
- **双击安装器源码**：一次打包三个平台，选择本机 CAD 后配置当前用户自动加载。

操作说明：[图片签章](docs/图片签章版使用说明.md) · [修订4功能对照及限制](docs/修订4-功能对照与使用.md) · [双击安装](docs/双击安装说明.md)

## 快速使用

1. 使用本版双击安装包选择目标 CAD，安装后重启 CAD；或将对应平台的完整构建目录保存到固定位置，执行 `NETLOAD` 加载 DLL，再选择菜单“安装自动加载”。无需每次手动加载。
2. 在“新增图框”中选择图框块，设置打印范围、图名/图号、纸张和**签章区域**。已有图框可在图框库编辑。
3. 打开“批打到文件（图框型）”，扫描图框，在“印章库”录入并选择图片、启用签章，选择 PDF 或 PNG/JPG 后打印。
4. 普通矩形边框使用“批打到文件（通用型）”，可调整纸张、命名、排序和小图框过滤；此入口不加章。

本次 GitHub 同步维护源码和文档；仓库不包含本机工程 DWG、个人印章或安装包二进制。安装器可按下文从源码生成。

## 验证范围

| 项目 | 当前证据与限制 |
| --- | --- |
| 中望机械2026 | 修订4实际出图回归通过：5种缩放/旋转图框、单页与合并PDF、PNG/JPG、上游16项及过滤命名回归 |
| AutoCAD2023 | 主要出图流程实测通过；最后一处过滤命名修复由共享源码及中望实测覆盖，最终重编译后未再次完整跑 AutoCAD |
| AutoCAD2015–2024其他年份 | 提供 net48 构建，未逐年宿主实测 |
| AutoCAD2025–2027 | 提供 net8 构建，未完成宿主实测 |
| 印章库和命名模块 | 13项检查通过，测试源码在 `tests/StampLibrary` |
| 安装器 | 解包、平台匹配及隔离注册表自测通过；不代表用户机器已经安装或完成安装后重启验证 |

本版聚焦批打到文件、图框库和图片签章；不承诺与 MSteel 全功能或全部界面一致。物理出纸、多个章、矢量签名库等不属于本轮验收范围。

## 从源码构建修订4

Windows x64，安装 .NET SDK 9（版本选择见 `global.json`），通过 NuGet 还原依赖：

```powershell
dotnet build LA.BatchPlot.sln -c Release
# 生成三个平台合一的安装器，输出到 release 目录
.\scripts\build-installer.ps1
# 可选：印章库/命名模块检查
dotnet build tests\StampLibrary\StampLibrary.csproj -c Release
.\tests\StampLibrary\bin\Release\net48\StampLibrary.exe "$env:TEMP\LA-StampLibrary-checks"
```

中望标准构建输出为 `bin`；AutoCAD 输出为 `bin-acad` 和 `bin-acad2025-2027`。加载时保留输出目录中的全部依赖及 Plotters，不要只复制主 DLL。宿主探针源码位于 `tests/StampSmoke` 和 `tests/AcadSmoke`，运行需要相应 CAD；构建成功不能代替宿主验证。

---

以下保留上游功能与详细操作参考；旧教程截图可能与修订4不同，新增功能及验证状态以上文为准。
...
手动加载方式：解压后用 `NETLOAD` 加载，再点击菜单「安装自动加载」。
...
| [docs/RELEASE_NOTES_v1.15.7.5.md](docs/RELEASE_NOTES_v1.15.7.5.md) | 当前版本发布说明 |
...
```powershell
.\scripts\package-release.ps1 -Version 1.15.7.5
```

输出位于 `release\v1.15.7.5\`，包含 ZWCAD、AutoCAD 2015–2024、AutoCAD 2025–2027 三组完整安装目录和对应压缩包。
