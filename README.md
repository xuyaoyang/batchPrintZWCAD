...
面向中望 CAD 和 AutoCAD 的批量打印插件。本仓库维护基于 [Edwardhehe/batchPrintZWCAD](https://github.com/Edwardhehe/batchPrintZWCAD) v1.15.7.5 的扩展版本，保留上游历史、作者信息和 MIT 许可。当前扩展版本：**2026-09-22 修订8**（程序集基础版本仍为 1.15.7.5）。

## 修订8：合并PDF自定义名称

图框型和通用型批打勾选“合并PDF”后，点击开始打印先弹出保存窗口，可指定合并文件名称和目录；取消则不开始打印。默认建议源DWG名称，可改成项目/版本名称。同名合并文件自动追加序号，保留已有版本，不受普通单页PDF的覆盖设置影响；按纸张分组合并时仍在自定义名称后追加图幅。打印期间若目标文件被其他程序创建，合并报错并保留已有文件，不删除覆盖。

三平台构建及安装器隔离自测通过；tests/MergeNaming验证同名计划、写入冲突保留原文件、自定义文件名、真实PDF页面及临时文件清理、带序号输出保留旧文件，6项通过。本轮未做CAD宿主窗口交互与完整批打验收。

## 修订7修复

补齐修订6漏测的四条独立直线图框：旧几何识别器要求每条线的包围盒同时有宽和高，错误丢弃水平/竖直边，造成无法识别实时外框并回退旧模板。现在先汇总有限坐标的线段范围，再对合并外框检查面积，整体缩放逻辑因此能正确工作。

新增直线/闭合多段线两类图框和0.7倍比例的同一测试：修复前12/32组失败，修复后32/32组、96项断言通过。AutoCAD 2023真实问题图纸副本验证：3个A3均恢复5880×4158范围及图号图名，20个A4结果保持不变；业务图纸和提取报告不上传。三平台编译和安装器自测通过；中望与AutoCAD 2025–2027未做本轮宿主实测，未完成完整输出打印验收。见[回归记录](tests/evidence/frame-scaling-r7/README.md)。

## 修订6修复

同一固定图框模板在不同 DWG 中整体放大或缩小时，扫描使用当前块定义内识别出的等比例外框，按新旧外框尺寸换算图号、图名、其他字段和签章区域；块插入比例、旋转仍只计算一次。编辑图框、套用已有图框及图号更新使用相同字段换算。原有 Frame 图框库无需重新录入，原 DWG 不因扫描/签章而改变。

AutoCAD 2023 后台宿主合成图回归：同一最小测试修复前 8/12 组失败，修复后 12/12 组通过（36项边界、字段、签章断言），覆盖块内 0.5/1/2 倍缩放、1/2 倍插入比例、0/90°旋转和非零局部原点。证据：[回归记录](tests/evidence/frame-scaling-r6/README.md)。三个平台构建通过；中望和 AutoCAD 2025–2027 本次未做宿主实测。

该自动适配针对外框长宽比一致的整体缩放（允许 0.5% 比例误差）。可拉伸模板保持右下角锚点规则，长宽比不同的手动打印范围不自动按固定模板缩放。历史 Local/World 模式仍沿用原坐标规则。
