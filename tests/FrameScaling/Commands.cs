...
public class FrameScalingTests
{
 ...
 public void Run()
 {
  ...
  foreach(var factor in new[]{1.0,0.5,2.0})
   ...
   try {
    using(var db=new Database(true,true)) {
     ...
     using(var tr=db.TransactionManager.StartTransaction()) {
      ...
      block.AppendEntity(pl);tr.AddNewlyCreatedDBObject(pl,true);
     ...
     results.Add(new{factor,insertion,angle,boundary,fields=job.Title=="TITLE"&&job.DrawingNumber=="NUMBER",stamp=stampOk,job.Title,job.DrawingNumber});
    }
   } catch(System.Exception ex) {results.Add(new{factor,insertion,angle,error=ex.ToString()});}
  }
